...
$cs->registerScript(uniqid(), '
	$(document).ready(function () {
		...
		$(document).on("click", ".lead-search-submit", function(e) {
			...
			$.ajax({
				url: yii.urls.absoluteUrl + "/agent/default2/leadSearch",
				type: "post",
				dataType: "json",
		...
		$(document).on("click", ".load-lead-to-hopper", function(e) {
			...
			if($(this).data("calling-time") == "Next Shift")
			{
			...
			{
				$.ajax({
					url: yii.urls.absoluteUrl + "/agent/default2/loadLeadToHopper",
					type: "post",
					dataType: "json",
					...
					success: function(response) {
						...
						if(response.status  == "success")
						{
							$(location).attr("href", yii.urls.absoluteUrl + "/agent/default2/index?action=nextLead&search_lead_id=" + response.search_lead_id);
						}
		...
		$(".manual-dial-btn").on("click", function() {
			...
			{
				$.ajax({
					url: yii.urls.absoluteUrl + "/agent/default2/manualDial",
					type: "post",
					dataType: "json",
					data: {
						"ajax": 1,
		...
		$("#agentStatsTab").on("click", function(){
			...
			$.ajax({
				url: yii.urls.absoluteUrl + "/agent/default2/loadAgentStats",
				type: "post",
				dataType: "json",
				data: { "ajax":1 },
				beforeSend: function(){ 
		...
		$(document).on("click", ".data-tab-submit-btn", function(){
			...
			$.ajax({
				url: yii.urls.absoluteUrl + "/agent/default2/updateDataTab",
				type: "post",
				dataType: "json",
				data: formData,
				beforeSend: function(){ 
		...
		$(document).on("change", ".data-tab-dropdown", function(){
			...
			$.ajax({
				url: yii.urls.absoluteUrl + "/agent/default2/ajaxLoadCustomData",
				type: "post",
				dataType: "json",
				data: { 		
					"ajax" : 1,	
