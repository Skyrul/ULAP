...
class DefaultController extends Controller
{
						...
						if(empty($existingleadHopperEntry))
						{
							$existingleadHopperEntry = LeadHopper::model()->find(array(
								'condition' => '
									...
									AND ( 
											(
												t.type = 2 AND ( t.callback_date <= NOW() AND DATE(t.callback_date) = DATE(NOW()) )
											) 
											OR (t.type = 5) 
										)
				...
				if( $leadHopperEntry )
				{
					...
					if( $leadHopperEntry->save(false) )
					{
						if( $leadHopperEntry->type != LeadHopper::TYPE_LEAD_SEARCH && !isset($_GET['ajax']) )
						{
							...
							//temporary override for mountain
							if( $leadHopperEntry->customer_id == 1021 )
							{
								$leadHopperEntry->agent_account_id = $authAccount->id;
							...
							else
							{
								if( $currentAssignedToAgent >= 1 )
								{
									...
									else
									{
										...
										LeadHopper::model()->updateAll(array('agent_account_id' => $authAccount->id), 'type = 1 AND lead_id = ' . $leadHopperEntry->lead_id);
									}
		...
		if( isset($_POST['ajax']) )
		{
			...
			if( $leadCallHistory->save(false) )
			{
				...
				if( $asterisk->call($asteriskParams) )
				{
					...
					{
						if( $customerQueueViewer->customer_id != '1021' )
						{
			...
			if( $leadCallHistory )
			{
				...
				if( $disposition )
				{
					...
					if(!empty($hopperEntry) && !empty($disposition) && !empty($leadCallHistory))
					{
						...
						if( $hopperEntryCurrentType == LeadHopper::TYPE_RESCHEDULE || $hopperEntryCurrentType == LeadHopper::TYPE_NO_SHOW_RESCHEDULE )
						{
							$hopperEntry->type = LeadHopper::TYPE_CONTACT;
						}
	...
	public function actionLoadLeadToHopper()
	{
		...
		if( isset($_POST['ajax']) && isset($_POST['id']) && isset($_POST['type']) )
		{
			...
			{
				$MsgHTML = '<p>Agent Name: ' . $authAccount->accountUser->getFullName().'</p>';
				...
				$mail->AddAddress('callfloor@engagex.com');
