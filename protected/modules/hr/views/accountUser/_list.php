...
<div class="tabbable tabs-left">
	...
	<div class="tab-content">
		...
		<div class="row">
						...
						<div class="col-md-4 text-right">
							<div id="nav-search" class="nav-search" style="position:inherit; margin-top:2px; right:0; ">
								<span class="input-icon">
									<input type="text" autocomplete="off" class="nav-search-input employee-search-input" placeholder="Search Employees..." style="width:200px;">
			...
			<div class="col-md-7 org-chart-wrapper"> 

				<div class="page-header">
					<h1 class="text-center">
						<a href="javascript:void(0);" class="pull-left collapse-user-list">
							<i class="fa fa-arrow-left"></i>
						</a>
						
						Engagex Org Chart
						<?php 
							if( Yii::app()->user->account->checkPermission('employees_settings_button','visible') )
							{
								echo CHtml::link('<i class="fa fa-cog"></i> Settings',array('orgChart/index'),array('class'=>'btn btn-sm btn-primary')); 
							}
						?>
					</h1>
				</div>
				
				<div class="org-tree" style="overflow-x:auto;">
					<ul>
						<?php 
							function showNested( $parentID )  
							{
								...
								if( $positions )
								{
									echo '<ul>';
										...
										{
											echo '<li>';
												echo '<a href="'.Yii::app()->createUrl('/hr/accountUser/employeeProfile', array('id'=>$position->account_id)).'">';
												
													echo $position->account->accountUser->job_title;
													
													echo '<br>';
													
													if($position->account->accountUser->getImage())
													{
														echo CHtml::image($position->account->accountUser->getImage(), '', array('class'=>'thumbnail'));
													}
													else
													{
														echo '<div style="width:100px; height:84px; border:1px dashed #ccc; text-align:center; margin:0 auto;">No Image Uploaded.</div>';
													}
													
													echo '<br>';
													
													echo $position->account->accountUser->getFullName();

												echo '</a>';
												
												showNested( $position->id );  
