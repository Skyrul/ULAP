...
<?php  ### customer email setting scripts
	if(!empty($tab) && ($tab == 'emailSetting' || $tab == 'emailSettingCreate' || $tab == 'emailSettingUpdate'))
	{
		...
		$cs->registerScriptFile($baseUrl . '/js/customer/customer_skill_multiple_uploader.js');
		...
		$cs->registerScript(uniqid(), '
			...
			$(document).ready( function(){
				...
				$(document).on("click", ".existing-remove-file-link", function(){
					...
					{
						$.ajax({
							url: yii.urls.absoluteUrl + "/customer/emailSetting/deleteEmailAttachment",
							type: "post",
							dataType: "json",
							data: { "id": id },
							success: function(response) { 
				...
				$(document).on("change", "#CustomerSkillEmailTemplate_is_sending_option_default", function(){
				...
				$("#CustomerSkillEmailTemplate_is_sending_option_default").trigger("change");
			});
...
<div class="row">
	<div class="col-sm-12">
		...
		<div class="tabbable tabs-left">
				...
				<?php						
					...
					if( !empty($selectedCustomerSkills) )
					{
						...
						foreach( $selectedCustomerSkills as $selectedCustomerSkill )
						{
							...
							if(empty($customer_skill_id))
							{
								if($ctr == 1)
									$active = 'active';
							}
							
							if($customer_skill_id == $selectedCustomerSkill->id)
								$active = 'active';
			...
			<div class="tab-content office-tab-content" style="display:<?php echo count($selectedCustomerSkills) > 0 ? 'block' : 'none'; ?>;">
					...
					if( !empty($selectedCustomerSkills) )
					{
						...
						foreach( $selectedCustomerSkills as $selectedCustomerSkill )
						{
							...
							<div style="min-height:200px;" class="tab-pane fade in <?php echo $active; ?>" id="customer-skill-<?php echo $selectedCustomerSkill->id; ?>">
								...
								<h4 style="color:#2679b5;"><?php echo $selectedCustomerSkill->skill->skill_name; ?></h4>
								<div class="tabbable tabs-left">
									<ul class="nav nav-tabs" role="tablist">
										<?php if(Yii::app()->user->account->checkPermission('customer_skills_contract_tab','visible')){ ?>
										<li role="presentation" class="<?php echo empty($tab) ? 'active' : ''; ?>">
										...
										<?php //if( Yii::app()->user->account->checkPermission('customer_skills_email_setting_tab','visible') && $customer->company->customer_specific_skill_scripts == 1 ){ ?>
										<li role="presentation" class="<?php echo !empty($tab) && ($tab == 'emailSetting' || $tab == 'emailSettingCreate' || $tab == 'emailSettingUpdate') ? 'active' : ''; ?>">
											<a href="#customerSkill-emailSetting-<?php echo $selectedCustomerSkill->id; ?>" role="tab" data-toggle="tab">Email Setting</a>
										</li>
										<?php //} ?>
									...
									<div class="tab-content">
										...
										<div role="tabpanel" class="tab-pane" id="customerSkill-customerCallSchedule-<?php echo $selectedCustomerSkill->id; ?>">
											<div class="col-md-12">
										...
										<?php 
											...
											$contract_is_shown = false;
											
											if(empty($tab))
												$contract_is_active = true;
											
											if(Yii::app()->user->account->checkPermission('customer_skills_contract_tab','visible'))
												$contract_is_shown = true;
											
										?>
										<div role="tabpanel" class="tab-pane <?php echo ($contract_is_active) ? 'active' : ''; ?>" id="customerSkill-contract-<?php echo $selectedCustomerSkill->id; ?>" style="<?php echo ($contract_is_shown) ? '' : 'display:none;'; ?>">
									...
									<?php #######--------Email Setting Render Partial via $tab param  # ?>
									
										<div role="tabpanel" class="tab-pane <?php echo !empty($tab) && $tab == 'emailSetting' ? 'active' : ''; ?>" id="customerSkill-emailSetting-<?php echo $selectedCustomerSkill->id; ?>">

											<?php $this->forward('/customer/emailSetting/index/customer_skill_id/'.$selectedCustomerSkill->id.'/customer_id/'.$customer->id,false); ?>
										</div>  
										
										<?php if($tab == 'emailSettingCreate'){ ?>

											<div role="tabpanel" class="tab-pane <?php echo !empty($tab) && $tab == 'emailSettingCreate' ? 'active' : ''; ?>" id="customerSkill-emailSetting-create-<?php echo $selectedCustomerSkill->id; ?>">
																						
												<?php $this->forward('/customer/emailSetting/create/customer_skill_id/'.$selectedCustomerSkill->id.'/customer_id/'.$customer->id,false); ?>
											</div> 
										<?php } ?>


										<?php if($tab == 'emailSettingUpdate'){ ?>
											<div role="tabpanel" class="tab-pane <?php echo !empty($tab) && $tab == 'emailSettingUpdate' ? 'active' : ''; ?>" id="customerSkill-emailSetting-update-<?php echo $selectedCustomerSkill->id; ?>">
																						
												<?php $this->forward('/customer/emailSetting/update/customer_skill_id/'.$selectedCustomerSkill->id.'/customer_id/'.$customer->id,false); ?>
											</div> 
										<?php } ?>
										
										<?php ########------END OF Email Setting Render Partial via $tab param ---- ?>
									</div>
								</div>
