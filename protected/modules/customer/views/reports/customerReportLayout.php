...
<table class="customer-info">
	...
	<tr>
		<td align="left">Report Range: <?php echo date('m/1/Y'); ?> - <?php echo date('m/t/Y'); ?></td>
	</tr>
		...
		<td align="left">
			...
			<?php 
				$callableLead = CustomerCallableLeadCount::model()->find(array(
					'condition' => 'customer_id = :customer_id AND YEAR(date_created) = YEAR(NOW()) AND MONTH(date_created) = MONTH(NOW())',
					'params' => array(
						':customer_id' => $customer->id
					),
				));
		...
		if( $dispositions )
		{
			foreach( $dispositions as $disposition )
			{
				$dispositionCount = LeadCallHistory::model()->count(array(
					'condition' => 'disposition = :disposition AND customer_id = :customer_id AND t.status!=4 AND t.lead_id IS NOT NULL AND t.date_created >= "'.date('Y-m-01 00:00:00').'" AND t.date_created <= "'.date('Y-m-t 23:59:59').'"',
					'params' => array(
