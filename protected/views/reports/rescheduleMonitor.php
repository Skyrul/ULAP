...
<div class="page-header">
	<h1>
		...
		<a class="btn btn-mini btn-primary" href="<?php echo $this->createUrl('reschedulesLoadALL'); ?>">Load All</a>
		<a class="btn btn-mini btn-danger" href="<?php echo $this->createUrl('reschedulesForceALL'); ?>">Force All</a>
	</h1>
</div>
	...
	<div class="row">
		<div class="col-sm-12">
			...
			<table id="leadsTbl" class="table table-striped table-bordered table-condensed table-hover">
				<thead>
					<th class="center">#</th>
					<th>Reschedule Type</th>
					<th>Reschedule Date</th>
					<th>Reschedule Status</th>
					<th>Disposition Dials</th>
					<th>Customer Name</th>
					<th>Lead Name</th>
					...
					if( $models )
					{
						...
						foreach( $models as $model )
						{
							...
							if( $status == 'Active' )
							{
								...
								if( count($disposAfterConflict) !== count(array_unique($completedConflictDispos)) )
								{
								?>
									...
									<tr>
										...
										<td><?php echo $model->calendarAppointment->title; ?></td>
										
										<td class="center"><?php echo date('m/d/Y', strtotime($model->calendarAppointment->date_updated)); ?></td>
										
										<td class="center">
											<?php 
												switch( $model->calendarAppointment->status )
												{
										...
										<td><?php echo $model->calendarAppointment->start_date; ?></td>
										...
										<td class="center">
											<?php 
												...
												{
													echo CHtml::link('Load to Queue <i class="fa fa-arrow-right"></i>', array('rescheduleAddToQueue', 'calendar_appointment_id'=>$model->id), array('class'=>'btn btn-mini btn-primary'));
												}
												else
												{
													echo CHtml::link('Force Lead <i class="fa fa-arrow-right"></i>', array('rescheduleAddToQueue', 'calendar_appointment_id'=>$model->id, 'force'=>1), array('class'=>'btn btn-mini btn-danger'));
												}
											?>
										</td>
									</tr> 
