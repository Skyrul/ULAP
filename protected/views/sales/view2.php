...
<div class="siren-img" style="display:none;">
	<?php 
		// echo CHtml::image(Yii::app()->baseUrl.'/images/sales/siren.png','',array('class'=>'','style'=>'height:550px;')); 

		foreach($marqueeData as $mDatas)
		{
			$account = Account::model()->findByPk($mDatas['salesRepID']);
			
			if( $account )
			{
				$accountUser = $account->accountUser;
				
				if( $accountUser->getOriginalImage() )
				{
					echo '<span class="profile-picture">';
						echo CHtml::image($accountUser->getOriginalImage(), '', array('class'=>'img-responsive','style'=>'height:550px;'));
					echo '</span>';
				}
				else
				{
...
<audio style="display:none;" id="audio-siren" controls>
	...
	<?php
		// Yii::app()->baseUrl.'/images/sales/Police Siren-SoundBible.com-878640712.wav';
		...
		foreach($marqueeData as $mDatas)
		{
			...
			if( $account )
			{
				...
				{
					echo '<source src="'.Yii::app()->baseUrl.'/images/sales/Police Siren-SoundBible.com-878640712.wav'.'" type="audio/wav">';
				}
