 ...
 */
class CustomerSkillEmailTemplate extends CActiveRecord
{
	public $existingId;
	public $lead;
	public $customer;
	...
	{
		return '{{customer_skill_email_template}}';
	}
	...
	public function rules()
	{
		...
		return array(
			array('customer_id, customer_skill_id, template_name, is_sending_option_default', 'required'),
			array('template_name', 'nameExisting'),
			array('existingId', 'required','on'=>'cloneExisting'),
			array('customer_id, customer_skill_id, is_sending_option_default', 'numerical', 'integerOnly'=>true),
	...
	public function nameExisting($attribute,$params)
	{
		if(!empty($this->template_name))
		{
			...
			$cset = self::model()->find($criteria);
			if(!empty($cset))
				$this->addError($attribute, 'Template name already exist');
		}
	}
}
