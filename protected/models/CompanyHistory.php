 ...
 *
 * The followings are the available columns in table '{{company_history}}':
 * @property integer $id
...
class CompanyHistory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('model_id, company_id, user_account_id, type, status', 'numerical', 'integerOnly'=>true),
			array('page_name, ip_address', 'length', 'max'=>255),
			array('content, old_data, new_data, date_created, date_updated', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, model_id, company_id, user_account_id, content, page_name, old_data, new_data, type, status, date_created, date_updated', 'safe', 'on'=>'search'),
		);
	}
