 ...
 * @proparty tinyint is_possible_now_pending = 
	0 = default/pending can't be called, 
	1= completed can be called in dialer, 
 */
class Lead extends AuditCActiveRecord
{
	public $ctr;
	public $_memberNumber;
	//type = 1 , valid number 
	...
	public function rules()
	{
		...
		return array(
			...
			array('last_call_date, timezone, recycle_date, recertify_date, specific_date, date_created, date_updated', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, list_id, customer_id, first_name, last_name, gender, partner_first_name, partner_last_name, home_phone_label, home_phone_number, home_phone_disposition, home_phone_disposition_detail, home_phone_dial_count, mobile_phone_label, mobile_phone_number, mobile_phone_disposition, mobile_phone_disposition_detail, mobile_phone_dial_count, office_phone_label, office_phone_number, office_phone_disposition, office_phone_disposition_detail, office_phone_dial_count, email_address, address, address2, address3, city, state, zip_code, language, agent_name, custom_date, number_of_dials, last_call_date, timezone, type, status, date_created, date_updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'customDataMemberNumber' => array(
				self::HAS_ONE, 'LeadCustomData', 'lead_id', 
				'condition' =>'customDataMemberNumber.field_name = "Member Number"', 
			),
		);
	}
	...
	public static function items2($customer_id)
	{
		...
		if( $customer )
		{
			...
			if( $customer->company_id == 9 ) //farmers
			{
				$addedCondition = ' AND t.is_possible_now_pending = 1';
			}
