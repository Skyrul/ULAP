...
class CronQueueCustomerController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if( time() < strtotime('today 6:00 am') )
		{
			echo date('g:i A');
			exit;
		}
		...
		$cronCustomerSkillOngoing = CustomerQueueViewerSettings::model()->findByPk(8);
		$cronCustomerSkillCurrentOffset = CustomerQueueViewerSettings::model()->findByPk(9);
		
		$cronCustomerSkillRecord = CustomerQueueViewerSettings::model()->findByPk(10);
		$cronCustomerSkillRecordChecker = CustomerQueueViewerSettings::model()->findByPk(11); 
		...
		// if( $cronCustomerSkillRecord->value == $cronCustomerSkillCurrentOffset->value && $cronCustomerSkillOngoing->value == 1 )
		// {
			// $cronCustomerSkillRecordChecker->value = $cronCustomerSkillRecordChecker->value + 1;
			// $cronCustomerSkillRecordChecker->save(false);
		// }
		
		// if( $cronCustomerSkillRecordChecker->value >= 1 )
		// {
			// $cronCustomerSkillOngoing->value = 0;
			// $cronCustomerSkillOngoing->save(false);
			
			// $cronCustomerSkillCurrentOffset->value = $cronCustomerSkillCurrentOffset->value+1;
			// $cronCustomerSkillCurrentOffset->save(false);
		// }
		
		// if( $customerSkills && $cronCustomerSkillOngoing->value == 0 && $cronCustomerSkillCurrentOffset->value < $customerSkillsCount )
		if( $cronCustomerSkillOngoing->value == 0 )
		{
			$cronCustomerSkillOngoing->value = 1;
			$cronCustomerSkillOngoing->save(false);
			...
			if( $customerSkills )
			{
				foreach( $customerSkills as $customerSkill )
				{
					...
					// $cronCustomerSkillRecord->value = $cronCustomerSkillCurrentOffset->value;
					// $cronCustomerSkillRecord->save(false);
					
					// $cronCustomerSkillRecordChecker->value = 0;
					// $cronCustomerSkillRecordChecker->save(false);
		...
		// if( $cronCustomerSkillCurrentOffset->value == $customerSkillsCount )
		// {
			// $cronCustomerSkillCurrentOffset->value = 0;
			// $cronCustomerSkillCurrentOffset->save(false);
		// }
