...
class CronQueueViewerGamingController extends Controller
{
				...
				if( $customerSkill )
				{
					...
					if( $customerQueue->next_available_calling_time == 'Now' )
					{
						...
						if( $customer->status == 1 && $customerIsCallable )
						{
							if( $lists )
							{
								foreach( $lists as $list )
								{
									...
									if( in_array($customerSkill->skill_id, array(36,37,38,43)) && $leadCounter < 2000 )
									{
										$leadCounter = 0;
									}
						...
						else
						{
							...
							echo '<b>Deleted Leads of Customer ID: ' . $customerQueue->customer_id.' - '.$customerQueue->customer_name.'</b>';
							echo '<br>';
				...
				if( !empty($timeZone) )
				{
					...
					if( $type == 'customer' )
					{
						...
						if( $customer->id == 2011 )
						{
							...
							if( time() >= strtotime('today 8:00 am') && time() < strtotime('today 8:00 pm') )
							{
								$nextAvailableCallingTime = 'Now';
