...
class ManualQueueEvaluationController extends Controller
{
	...
	public function actionIndex()
	{
		...
		// {
			...
			// {
				$existingQueueViewers = CustomerQueueViewer::model()->findAll(array(
					// 'condition' => 'next_available_calling_time NOT IN ("On Hold", "Cancelled", "Removed")', 
					// 'limit' => 50,
					// 'offset' => 100,
					// 'condition' => 'customer_id IN (2828, 2374)',
					// 'condition' => 'customer_id IN(568,1012,1085,1059,973,880,1026,556,841,850,145,1086,998,1135,458,1103,87,643,94,960,941,1108,667,994,127,604,979,548,1033,1089,495,1051,615,427,626,759,831,809,808,1055,520,864,1044,1123,1048,1047,789,560,465,965,1179,569,1,108,1060,1003,1039,140,1145,968,733,159,592,1056,1099,984,1083,1109,915,797,920,449,645,931,652,765,122,648,1095,1102,983,678,1006,910,1040,860,88,1096,1028,163,900,878,441,925,1071,796,840,976,410,1061,771,1113,1065,975,1070,1066,898,905)',
					// 'condition' => 'customer_id = 2607',
					// 'condition' => 'fulfillment_type IN ("GOAL", "LEAD") AND customer_id NOT IN (499, 1165)',
					// 'condition' => 'status=1 AND priority >=0 AND priority <=.24 AND available_leads > 0 AND next_available_calling_time NOT IN ("Goal Appointment Reached", "On Hold", "Cancelled", "Removed")',
					// 'condition' => '(priority = 1 AND available_leads > 0 ) OR (priority >= 0.75 AND priority <= 0.99 AND available_leads > 0)',\
					// 'condition' => 'priority >= 0.75 AND priority <= 0.99 AND available_leads > 0',
					// 'condition' => 'priority >= 0.50 AND priority <= 0.74 AND available_leads > 0',
					// 'condition' => 'priority >= 0 AND priority <= 1',
					// 'condition' => 'priority=0',
					// 'condition' => 'enrollment_date IS NULL',
					// 'condition' => 'next_available_calling_time NOT IN ("On Hold", "Cancelled", "Removed")',
					// 'condition' => 'priority_reset_date="03-1-2017"',
					// 'condition' => 'priority_reset_date="05-1-2017"',
					// 'order' => 'priority DESC',
				)); 
			// }
			
			echo 'count: ' . count($existingQueueViewers);
					...
					if( $type == 'customer' )
					{
						...
						if( $customer->id == 1966 )
						{
							...
							else
							{
								...
								if( in_array($this->get_timezone_abbreviation($timeZone), array('CST', 'CDT')) )
								{
									if( time() >= strtotime('today 9:00 am') && time() < strtotime('today 7:00 pm') )
									{
										$nextAvailableCallingTime = 'Now';
									}
									else
								...
								if( in_array($this->get_timezone_abbreviation($timeZone), array('MST', 'MDT')) )
								{
									if( time() >= strtotime('today 10:00 am') && time() < strtotime('today 8:00 pm') )
									{
										$nextAvailableCallingTime = 'Now';
									}
									else
								...
								if( in_array($this->get_timezone_abbreviation($timeZone), array('PST', 'PDT')) )
								{
									if( time() >= strtotime('today 11:00 am') && time() < strtotime('today 9:00 pm') )
									{
										$nextAvailableCallingTime = 'Now';
									}
									else
