			...
			{ 
				...
				if( $model->save() )
				{
					if( $model->lead_id != null )
					{
						...
						if( isset($_POST['approved']) || isset($_POST['declined']) || isset($_POST['suggest']) || in_array($model->title, array('CANCEL APPOINTMENT', 'RESCHEDULE APPOINTMENT', 'CHANGE APPOINTMENT', 'UPDATE CALENDAR ONLY')) )
						{
						...
						{
							if( in_array($model->title, array('NO SHOW RESCHEDULE', 'RESCHEDULE APPOINTMENT')) )
							{
								$existingLeadHopperEntry = LeadHopper::model()->find(array(
									'condition' => 'lead_id = :lead_id',
									'params' => array(
			...
			else
			{
				...
				if( $model->save() )
				{
					if( $model->lead_id != null )
					{
						...
						$leadCallHistory = LeadCallHistory::model()->find(array(
							// 'condition' => 'lead_id = :lead_id AND calendar_appointment_id = :calendar_appointment_id',
							'condition' => 'lead_id = :lead_id AND calendar_appointment_id IS NULL',
							'params' => array(
								':lead_id' => $model->lead_id,
								// ':calendar_appointment_id' => $model->id,
							),
...
/* Start of Private Functions */
	...
	private function createConfirmationCall($model)
	{
		...
		if( $confirmationCall->save(false) )
		{
			...
			if( $model->title != 'RESCHEDULE APPOINTMENT' )
			{
				$leadHistory->setAttributes(array(
					'lead_id' => $model->lead_id,
					'lead_phone_number' => $leadPhoneNumber,
