$(function() {
	...
	uploader = new plupload.Uploader({
		...
		browse_button : "plupload-select-voice",
		container: "voice-sources",
		max_file_size : maxFileSizeAllowed,
		multi_selection: true,
		chunk_size: "1mb",
		unique_names: true,
		file_data_name : "FileUpload[filename]",
		url : yii.urls.absoluteUrl + "/customer/data/uploadVoice",
	...
	uploader.bind("Init", function(up, params) {
		$("div#voice-sources a#plupload-select-voice").html("<i class=\"icon-plus\"></i> Attach a File");
	});
	...
	uploader.bind("FilesAdded", function(up, files) {
		...
		{
			for (var i in files) {
				$("div#voice-sources .filelist").append("\
	...
	uploader.bind("FileUploaded", function(up, file, info) {
		...
		$("#voice-container").html("<audio controls src='"+yii.urls.baseUrl+"/voice/"+response.generatedFilename+"'></audio>");
	...
	uploader.bind('BeforeUpload', function(up) {
		up.settings.url = yii.urls.absoluteUrl + "/customer/data/uploadVoice?event_id=" + event_id;
	});
