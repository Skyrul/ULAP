	...
	$(document).ready( function(){ 
		...
		var calendar = $('#calendar').fullCalendar({
			...
			events: function(start, end, timezone, callback) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						$(".load-calendar-btn").html("Load");
						$(".load-calendar-btn").prop("disabled", false);
					}
				});
			},
					...
					$.ajax({
						...
						success: function(response) {
							...
							modal.on("change", "#CalendarAppointment_title", function(){
								...
								{
									modal.find(".add-new-lead-link").fadeIn();
								...
								{
									modal.find(".add-new-lead-link").hide();
							...
							modal.find('button[data-action=save]').on('click', function() {
								...
								if( modal.find("#CalendarAppointment_title").val() == "APPOINTMENT SET" || modal.find("#CalendarAppointment_title").val() == "INSERT APPOINTMENT" )
								{
									if( modal.find("#div.profile-info-value CalendarAppointment_lead_id").val() == "" )
									{
										...
										success: function(response) {
											...
											{													
												if( typeof hasMadeCalendarChanges != 'undefined' )
												{
													...
													//update both LINE 319 AND 891 for disposition narrowing
													var dispoSelectDropdown = $(".dispo-select:enabled");
													var calAppointmentTitleVal = modal.find("#CalendarAppointment_title").val();
													
													
													$(".dispo-select:enabled option").each(function(index, option) {
															$(option).hide();
														
														if(calAppointmentTitleVal == "APPOINTMENT SET" && $(this).attr("is_appointment_set") == 1)
														{
															$(option).show().prop("selected",true);
															dispoSelectDropdown.trigger("change");
														}
														
														if(calAppointmentTitleVal == "LOCATION CONFLICT" && $(this).attr("is_location_conflict") == 1)
														{
															$(option).show().prop("selected",true);
															dispoSelectDropdown.trigger("change");
														}
														
														if(calAppointmentTitleVal == "SCHEDULE CONFLICT" && $(this).attr("is_schedule_conflict") == 1)
														{
															$(option).show().prop("selected",true);
															dispoSelectDropdown.trigger("change");
														}
			...
			eventResize: function(calEvent, delta, revertFunc) {
				...
				if( !blackOutFormOngoing )
				{
					blackOutFormOngoing = true;				
						...
						success: function(response) {
							...
							if(response.html != "")
							{
								...
								modal.find('button[data-action=save]').on('click', function() {
									...
									action = modal.find('#formAction').val();
									
									$.ajax({
										url: yii.urls.absoluteUrl + '/calendar/' + action,
										type: 'post',
										dataType: 'json',
										data: data,
										success: function(response) {
			...
			eventRender: function (event, element, icon) {
				...
				var current_date = moment(event.start).format('YYYY-MM-DD HH:mm:ss');
				...
				if( event.title != 'BLACKOUT DAYS' && event.custom != 3 && viewer == 'customer' )
				{
					if(check >= today)
					...
					element.find(".delete-slot").click(function() {
						...
						if( confirm("Are you sure you want to remove this?") )
						{
							$.ajax({
								...
								type: 'post',
								dataType: 'json',
								data: { "ajax": 1, "event_id": event.id, "current_date":current_date, "calendar_id": current_calendar_id },
								success: function(response) {
									if( response.status == "success" )
									{					
			...
			eventClick: function(calEvent, jsEvent, view) {
				...
				if( viewer == 'agent' )
				{
					...
					if( calEvent.custom == 3 && calEvent.color == '#D15B47' && (calEvent.status == 2 || calEvent.status == 3) )
					{
						...
						{
							alert_message = 'No action is made yet for this conflict.';
						}
						
						if( calEvent.status == 3 )
						{
							alert_message = 'Conflict is declined.';
						}
				...
				if( calEvent.custom == 4 )
				{
					action = 'actionFormAvailableSlot'
				}
				...
				{		
					...
					$.ajax({
						...
						data: { 
							...
							"calendar_id": current_calendar_id,
							"start_date": moment(calEvent.start).format('YYYY-MM-DD HH:mm:ss'),
							"end_date": moment(calEvent.end).format('YYYY-MM-DD HH:mm:ss')
						},
						success: function(response) {
							...
							modal.on("change", "#CalendarAppointment_title", function(){
								...
								if( viewer == 'customer' )
								{
									...
									else if( $(this).val() == "RESCHEDULE APPOINTMENT" )
									{
										...
										modal.find('.alt-date-field-container').hide();
										modal.find('.start-time-field-container').hide();
										modal.find('.end-time-field-container').hide();
								...
								$.ajax({
									...
									success: function(response) {
										...
										modal.find("#CalendarAppointment_agent_notes").val(response.notesPrefill);
							...
							modal.find('button[data-action=save]').on('click', function() {
								...
								if( modal.find("#CalendarAppointment_title").val() == "" )
								{
							...
							modal.find('button[data-action=suggest]').on('click', function() {
								...
								modal.find("#CalendarAppointment_alt_date").prop("disabled", false);
								modal.find("#CalendarAppointment_start_date_time").prop("disabled", false);
								modal.find("#CalendarAppointment_end_date_time").prop("disabled", false);
