$(function() {
	...
	// Setup plupload
	flyerUploader = new plupload.Uploader({
		runtimes : "flash, html5, html4",
		browse_button : "plupload-select-files-flyer",
		container: "sources-flyer",
		max_file_size : maxFileSizeAllowed,
		multi_selection: true,
		chunk_size: "1mb",
		unique_names: true,
		file_data_name : "FileUpload[filename]",
		url : yii.urls.absoluteUrl + "/admin/company/flyerUpload",
	...
	flyerUploader.bind("Init", function(up, params) {
		$("div#sources-flyer a#plupload-select-files-flyer").html("<i class=\"fa fa-upload\"></i> Upload a File");
	});
	
	flyerUploader.init();

	flyerUploader.bind("FilesAdded", function(up, files) {
		var fileUploadCount = up.files.length;
		var error = "";
		
		if (maxFileCount > 0)
		{
			if ($("div.filelist-flyer").children().length >= maxFileCount || fileUploadCount > maxFileCount)
			{
		...
		{
			for (var i in files) {
				$("div#sources-flyer .filelist-flyer").append("\
					<div class=\"selected-file-flyer computer-upload\" id=\"" + files[i].id + "\">\
						...
						<div class=\"upload-info well\">\
							...
							<a class=\"remove-file-link-flyer\" href=\"#\"><i class=\"icon-remove\"></i> Remove</a>\
		...
		else
		{
			...
			for (var i in files) {
				file = flyerUploader.getFile(files[i].id);
				flyerUploader.removeFile(file);
			}
		}
	});
	
	flyerUploader.bind("UploadProgress", function(up, file) {
		$("#" + file.id).find(".percentage").html("(" + file.percent + "%)");
		$("#" + file.id).find(".progress .bar-wrapper div.bar").css("width", file.percent + "%");
	});
	
	flyerUploader.bind("FileUploaded", function(up, file, info) {
		var response = $.parseJSON(info.response);
		
		console.log(response);
		$("#photo-container-flyer").html("<img src='"+yii.urls.baseUrl+"/fileupload/"+response.generatedFilename+"' style='width:100%;'>");
	...
	flyerUploader.bind('BeforeUpload', function(up) {
		up.settings.url = yii.urls.absoluteUrl + "/admin/company/flyerUpload?event_id=" + event_id;
	});
	
	
	flyerUploader.bind("QueueChanged", function(up) {
	...
	flyerUploader.bind("StateChanged", function(up) {
		switch (up.state)
		{
	...
	flyerUploader.bind("Error", function(up, args) {
		var fileUploadCount = up.files.length;
	...
	$("#sources-flyer").on("click", ".remove-file-link-flyer", function(e) {
		e.preventDefault();
		...
		if (flyerUploader)
		{
			var file = flyerUploader.getFile(fileContainerId);
