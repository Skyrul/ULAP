...
{
    try{
    //var pgh = common.GetDeviceHeight() - j$('#acc_footer').height() - 3; j$('#page_accounts').css('min-height', pgh + 'px'); // must be set when softphone is skin in div
...
*/
        ...
        j$('#adialog_positive').on('click', function (event)
        {
            ...
            j$( '#addaccountpopup' ).on( 'popupafterclose', function( event )
            {
                ...
                //pwd = common.NormalizeInput(pwd, 0);
...
{
    try{
    common.PutToDebugLog(4, "EVENT, _accounts: onDestroy");
    global.isAccountsStarted = false;
    ...
    } catch(err) { common.PutToDebugLogException(2, "_accounts: onDestroy", err); }
}
